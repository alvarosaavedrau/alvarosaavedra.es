...
<Callout type="error">
    Proyecto actualmente en desuso.
</Callout>
...
La solución que he desarrollado consiste en una web: https://ranking.alvarosaavedra.es, la cual muestra un formulario donde los jugadores deben introducir su nombre, el nombre del contrincante, el resultado, etc.
...
1. Buscar un nombre de dominio más corto y fácil de recordar, como por ejemplo `rankingtenis.es`, `rankingcv.es`, `ranking-tenis-cv.es`, etc. hay una extensión `.tennis` que sería ideal para este caso pero cuesta ~52€ al año.